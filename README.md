...
Запуск приложения необходимо осуществлять с помощью команды `docker-compose up --build` при запуске с
локальной бд необходимо добавить параметр `-f docker-compose.dev.yml` перед `up`
